...
- [Examples](#examples)
    - [`basic`](#basic)
    - [`calculator_float32`](#calculator_float32)
    - [`calculator_int32`](#calculator_int32)
    - [`count`](#count)
    - [`display`](#display)
    - [`hello`](#hello)
    - [`keyboard`](#keyboard)
...
- __Display:__ writing an ASCII char (32-126) to `R19` and setting `R18` pushes it to the next cell of a 20x4 character grid, wrapping to a new (cleared) row once the current row is full and starting over on a cleared display once the last row is full (see `examples/display.kasm`). Writing a backspace (8) instead steps back onto the previous cell and clears it, which is how a program erases what it printed; at the start of a row it does nothing. The display clears `R18` once it has taken the char, and it does that on its own schedule rather than per instruction, so a program has to wait for `R18` to go back to 0 before it pushes the next char or the char is overwritten before the display ever sees it.
...
The display is 20 columns by 4 rows, which the interpreter does not try to work around, and two things follow from it. A typed line longer than 20 characters wraps onto the next row, and backspace does nothing at the start of a row -- the display has no cursor to step back over a wrap -- so a long line can be corrected only down to its last wrap. And `LIST` prints one row per program line onto four rows that clear themselves every fourth row, so anything but a very short program scrolls past before it can be read; the same goes for a `PRINT` in a loop. The 40 byte program record is untouched by this: it is storage, not a screen width, and it is what still bounds an expression to five terms and a string literal to 29 characters.
