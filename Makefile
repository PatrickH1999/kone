...
FREEROUTING_PASSES ?= 3
KICAD_DIR := logisim/kicad
...
logisim_clean:
	rm -f $(LOGISIM_CIRCS)
	rm -rf bin/logisim_test logisim/python/__pycache__ $(KICAD_DIR)
...
# error; unrouted nets are warnings, since routing is left to the fab step.
logisim_kicad:
	@$(TEST_COLORS); \
	python3 logisim/python/build_kicad.py; \
	passed=0; failed=0; \
	printf "\n"; \
...
# generated by logisim/python/logisim/kicad.py.
logisim_route: logisim_kicad
	@$(TEST_COLORS); \
	...
	for b in $(LOGISIM_BOARDS); do \
		d=$(KICAD_DIR)/$$b; \
		printf "routing %s, %s passes ...\n" "$$b" "$(FREEROUTING_PASSES)"; \
		java -Djava.awt.headless=true -jar $(FREEROUTING_JAR) \
			-de $$d/$$b.dsn -do $$d/$$b.ses -mp $(FREEROUTING_PASSES) \
			> $$d/route.log 2>&1; \
		python3 logisim/python/build_kicad.py --route $$b; \
		kicad-cli pcb drc --severity-error --exit-code-violations \
