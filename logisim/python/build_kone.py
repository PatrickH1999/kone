...
def wires(c, x, y, bus, names):
    ...
    split = c.add(
        Splitter(
            x, y, fanout=n, incoming=n, facing="north", appear="left", spacing=2
        )
    )
...
def mux4(c, x, y, tag, sel, srcs, out):
    ...
    for j in range(4):
        ...
        for half, k in ((1, 2 * j), (2, 2 * j + 1)):
            ...
            nets[f"{half}Y"] = out[k]
        wire_dip(
            c, c.add(Ttl74153(x + PITCH * j, y, label=f"{tag}{2 * j}")), nets
        )
...
def mux2(c, x, y, tag, sel, a, b, out):
    ...
    for j in range((len(out) + 3) // 4):
        ...
        wire_dip(
            c, c.add(Ttl74157(x + PITCH * j, y, label=f"{tag}{4 * j}")), nets
        )
...
def sequencer(rom, dispatch_a, dispatch_b):
    ...
    for i, (tag, table) in enumerate((("A", dispatch_a), ("B", dispatch_b))):
        ...
        stub(
            c, chip.port("data"), (x + 300, y + 60), f"D{tag}", "west", width=8
        )
    ...
    wire_dip(
        c,
        c.add(Ttl74151(4600, y, label="cond")),
        {
            "D0": Ground,
            "D1": "BUSZ",
            "D2": "NBUSZ",
    ...
    wire_dip(
        c,
        c.add(Ttl7427(5000, y, label="busz")),
        {
            "A1": "BUS0",
            "B1": "BUS1",
            "C1": "BUS2",
    ...
    wire_dip(
        c,
        c.add(Ttl7411(5400, y, label="busz")),
        {
            "A1": "Z0",
            "B1": "Z1",
            "C1": "Z2",
    ...
    wire_dip(
        c,
        c.add(Ttl7404(5800, y, label="inv")),
        {
            "A1": "BUSZ",
            "Y1": "NBUSZ",
            "A2": "BUS0",
...
def datapath():
    ...
    mux4(
        ...
        "bus",
        ("BSRC0", "BSRC1"),
        [bits("REGO"), bits("MEMO"), bits("ALUO"), bits("LIT")],
        bits("BUS"),
    )
    mux4(
        c,
    ...
    y += 800
    for i, (tag, q, en) in enumerate(
        (("t", "TQ", "nTW"), ("t2", "T2Q", "nT2W"), ("al", "ALQ", "nALW"))
    ):
        latch(c, 200 + PITCH * i, y, tag, bits("BUS"), bits(q), en)
    latch(
        c, 1400, y, "cy", ["ALUC"] + [Ground] * 7, ["CY"] + [None] * 7, "nCYW"
    )
    wire_dip(
        c,
        c.add(Ttl7404(1800, y, label="inv")),
        {
            "A1": "TW",
            "Y1": "nTW",
            "A2": "T2W",
...
def memory(program):
    ...
    c = Circuit("memory")
    y = ports(
        c, [("CLK", 1), ("BUS", 8), ("WE", 8)], [("MEMO", 8), ("MAR", 16)]
    )
    ...
    # nRAMWE is for the real SRAM on the board, whose write strobe is active
    # low; Logisim's RAM takes the active-high one.
    wire_dip(
        c,
        c.add(Ttl7404(1000, y, label="inv")),
        {
            "A1": "MLW",
            "Y1": "nMLW",
            "A2": "MHW",
    ...
    wire_dip(
        c,
        c.add(Ttl7408(1400, y, label="ramwe")),
        {
            "A1": "MEMW",
            "B1": "MA15",
            "Y1": "RAMWE",
    ...
    rom = c.add(
        Rom(
            800,
            y,
            addr_width=ROM_ADDR_BITS,
            data_width=8,
            label="prog",
            contents=image(program, ROM_ADDR_BITS),
        )
    )
...
def io():
    ...
    wire_dip(
        c,
        c.add(Ttl74138(200, y, label="devwr")),
        {
            "A": "ADDR0",
            "B": "ADDR1",
            "C": Ground,
    ...
    wire_dip(
        c,
        c.add(Ttl7411(600, y, label="devsel")),
        {
            "A1": "ADDR4",
            "B1": "NADDR3",
            "C1": "NADDR2",
    ...
    wire_dip(
        c,
        c.add(Ttl7427(1000, y, label="r18z")),
        {
            "A1": "R18Q0",
            "B1": "R18Q1",
            "C1": "R18Q2",
    ...
    wire_dip(
        c,
        c.add(Ttl7404(1400, y, label="inv")),
        {
            "A1": "ADDR2",
            "Y1": "NADDR2",
            "A2": "ADDR3",
    ...
    wire_dip(
        c,
        c.add(Ttl7408(1800, y, label="and1")),
        {
            "A1": "RW",
            "B1": "DEVSEL",
            "Y1": "DEVWR",
    ...
    wire_dip(
        c,
        c.add(Ttl7408(2200, y, label="and2")),
        {
            "A1": "nW18",
            "B1": "R18Z",
            "Y1": "nCLK18",
    ...
    wire_dip(
        c,
        c.add(Ttl7404(2600, y, label="inv2")),
        {
            "A1": "KBSET",
            "Y1": "NKBSET",
            "A2": "DISPCLR",
    ...
    for j in range(2):
        wire_dip(
            c,
            c.add(Ttl7408(3000 + PITCH * j, y, label=f"r18d{j}")),
            {
                f"{p}{i + 1}": v
                for i in range(4)
                for p, v in (
    ...
    mux4(
        ...
        "dev",
        ("ADDR0", "ADDR1"),
        [bits("R16Q"), bits("R17Q"), bits("R18Q"), bits("R19Q")],
        bits("DEVO"),
    )
...
if __name__ == "__main__":
    ...
    blocks = (
        regfile(),
        alu(),
        sequencer(rom, dispatch_a, dispatch_b),
        datapath(),
