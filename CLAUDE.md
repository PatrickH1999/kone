...
make circ       # every logisim/python/build_*.py -> logisim/*.circ
make cpu        # only kone.circ; PROG=bin/<name>.bin is the program in its ROM
make test-logisim   # boot four programs on kone.circ in Logisim, headless (~90 s)
...
`kone_microcode.py`. `make cpu` builds it, `PROG=bin/<name>.bin` picks the program baked into
its ROM, `make test-logisim` boots `display`, `hello`, `keyboard` and the `mem` klib test
headlessly and checks the TTY -- the klib one runs `mem_poke`, so it is also the proof that
the machine executes from RAM. It is
microcoded: every register the ISA names stays in the register file at the VM's index, seven
256-byte ROMs hold the microprogram and two more map an opcode to its entry point, which is
`cpu_decode_exec()`'s switch. What that costs, and what to know before touching it:
