...
def check_addr_alias(words: list[str], pc: int, addr_alias: dict[str, int], parse_cycle: str) -> tuple[dict[str, int], bool]:
    if (words[0][-1] == ":"):
        alias = words[0][:-1]
        if (parse_cycle == "addr_alias"): addr_alias[alias] = pc
        return addr_alias, True
    return addr_alias, False

def get_opcode(words: list[str], opcodes: dict[str, int], arglims: dict[str, list[int]]) -> str:
    opcode = words[0]
...
def get_arg(
    ...
    opcode: str
) -> int:   # Performs checks and converts opcode argument (uint8_str or address alias) to uint8
    arg = -1
    if (parse_cycle == "main"):
...
addr_alias = {}
machine_code = []
takes_addr_alias = ["JMP", "JC0", "JC1", "JA0", "JA1"] \
...
for parse_cycle in ["addr_alias", "main"]:
    ...
    for line in lines:
        ...
        if (len(words) == 0): continue   # No words in line -> go to next line
        addr_alias, found = check_addr_alias(words, pc, addr_alias, parse_cycle)
        if found: continue   # Address alias found -> go to next line
        ...
        if (len(words) == 1) and (parse_cycle == "main"):
            ...
            arg_str = words[1]
            arg = get_arg(arg_str, parse_cycle, addr_alias, arglims, takes_addr_alias, opcode)
            two_cycles = ((opcodes[opcode] >> 7) == 1)   # Flag: Opcode requires one/two cycles
            cycles = []
